...
class CustomerIO
{
    ...
    public function client(): Client
    {
        if ($this->clientInstance === null) {
            $this->clientInstance = new Client(
                $_SERVER['TINY_CIO_TRACK_API_KEY'] ?? '',
                $_SERVER['TINY_CIO_TRACK_SITE_ID'] ?? '',
                ['region' => $_SERVER['TINY_CIO_REGION'] ?? '']
            );
            $this->clientInstance->setAppAPIKey($_SERVER['TINY_CIO_APP_API_KEY'] ?? '');
        }
    ...
    {
        $endpoint = 'https://api' . ($_SERVER['TINY_CIO_REGION'] === 'eu' ? '-eu' : '') . '.customer.io/v1/send/email';
        ...
        curl_setopt_array($curl, [
            ...
            CURLOPT_HTTPHEADER => [
                'Authorization: Bearer ' . ($_SERVER['TINY_CIO_APP_API_KEY'] ?? ''),
