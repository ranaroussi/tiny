 ...
 * Usage:
 *
 * $ogimage = new OpenGraphImage(tiny::config()->app_path .'/'. tiny::config()->media_dir . '/ogfiles');
 * $ogimage->setTemplate('template.png', 500, 270, 640);
 * $ogimage->setTitle('My Title');
 * $ogimage->setDescription('My Description');
 * $ogimage->render(...);
 */
class OpenGraphImage
{
    ...
    public function render(int $quality = 80, ?string $file_path = null, ?string $format = 'webp'): void
    {
        ...
        // Output image
        if ($file_path) {
            $extension = strtolower(pathinfo($file_path, PATHINFO_EXTENSION)) ?? $format;
