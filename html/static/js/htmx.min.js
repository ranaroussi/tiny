...
!function(){var e=null;function t(t,r){if(t&&t.indexOf("<head")>-1){const v=document.createElement("html");var n=t.replace(/<svg(\s[^>]*>|>)([\s\S]*?)<\/svg>/gim,"").match(/(<head(\s[^>]*>|>)([\s\S]*?)<\/head>)/im);if(n){var a=[],d=[],o=[],h=[];v.innerHTML=n;var i=v.querySelector("head"),u=document.head;if(null==i)return;var m=new Map;for(const e of i.children)m.set(e.outerHTML,e);var s=e.getAttributeValue(i,"hx-head")||r;for(const t of u.querySelectorAll("script")){var l=m.has(t.outerHTML),c="re-eval"===t.getAttribute("hx-head"),g="true"===e.getAttributeValue(t,"hx-preserve");l||g?c?d.push(t):(m.delete(t.outerHTML),o.push(t)):"append"===s?c&&(d.push(t),h.push(t)):!1!==e.triggerEvent(document.body,"htmx:removingHeadElement",{headElement:t})&&d.push(t)}h.push(...m.values());for(const t of h){var p=document.createRange().createContextualFragment(t.outerHTML);!1!==e.triggerEvent(document.body,"htmx:addingHeadElement",{headElement:p})&&(u.appendChild(p),a.push(p))}e.triggerEvent(document.body,"htmx:afterHeadMerge",{added:a,kept:o,removed:d})}}}htmx.defineExtension("head-support",{init:function(r){e=r,htmx.on("htmx:afterSwap",(function(r){var n=r.detail.xhr.response;e.triggerEvent(document.body,"htmx:beforeHeadMerge",r.detail)&&t(n,r.detail.boosted?"merge":"append")})),htmx.on("htmx:historyRestore",(function(r){e.triggerEvent(document.body,"htmx:beforeHeadMerge",r.detail)&&(r.detail.cacheMiss?t(r.detail.serverResponse,"merge"):t(r.detail.item.head,"merge"))})),htmx.on("htmx:historyItemCreated",(function(e){e.detail.item.head=document.head.outerHTML}))}})}();
